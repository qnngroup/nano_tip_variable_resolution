...
tip = [mp.Cylinder(radius=trunk_radius,
                   center=mp.Vector3(0.0, -1*sY/4.0 - cone_height/2.0, 0.0),
                   height=(sY/2.0 - cone_height),
                   axis=mp.Vector3(0.0, 1.0, 0.0)),
       mp.Cone(center=mp.Vector3(0, -1*cone_height/2.0),
               height=cone_height,
               radius=trunk_radius,
               radius2=tip_radius,
               axis=mp.Vector3(0,1,0)),
       mp.Sphere(center=mp.Vector3(0,0,0),
                 radius=tip_radius)]
...
def mat_func(r):
    ...
    mat = mp.Medium(epsilon_diag = mp.Vector3(eps_diag[0,0], eps_diag[0,1], eps_diag[0,2]),
                           mu_diag=mp.Vector3(mu_diag[0,0], mu_diag[0,1], mu_diag[0,2]))
...
def my_amp_func_y(r):
    ...
    phase_factor = np.exp(-1*2*1j*np.pi*ky*r.y)
...
sim = mp.Simulation(cell_size=cell,
                    ...
                    extra_materials=[mp.Medium(epsilon=eps_tip,mu=4)],
                    verbose=True)


sim.run(mp.in_volume(monitor_xy, mp.to_appended("tEy_xy", mp.at_every(1/fcen/sample, mp.output_efield_y))),
        mp.in_volume(monitor_yz, mp.to_appended("tEy_yz", mp.at_every(1/fcen/sample, mp.output_efield_y))), until=waves/fcen)
